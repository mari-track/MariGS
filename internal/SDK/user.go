	...
	if account.Username == "" {
		...
		if s.Config.Account.AutoCreate {
			...
			if err != nil {
				...
				loginrsq.Data = nil
				loginrsq.Message = "MariGS\b自动注册账号失败\n请联系管理员"
			...
			rspaccount := &LoginAccount{
				...
				Name:          "",
				Email:         requestData.Account + "@MariGS.com",
				IsEmailVerify: "0",
				Token:         token,
				Country:       "HK",
			...
			loginrsq.Data = nil
			loginrsq.Message = "MariGS\b账号不存在\n已关闭自动注册\n请手动注册"
			loginrsq.Retcode = -107
			c.JSON(200, loginrsq)
			return
		}
...
func (s *Server) VerifyRequestHandler(c *gin.Context) {
	...
	rspaccount := &LoginAccount{
		UID:           strconv.Itoa(int(account.AccountId)),
		Name:          "",
		Email:         account.Username + "@MariGS.com",
		IsEmailVerify: "0",
		Token:         account.Token,
		Country:       "HK",
		IdentityCard:  "320************110",
...
func (s *Server) V2LoginRequestHandler(c *gin.Context) {
	...
	if account.Username == "" {
		...
		if account.Token == loginData.Token {
			...
			if err != nil {
				logger.Error("ComboToken更新失败:%s", err)
				return
			}
