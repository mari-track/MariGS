...
type AvatarSkillDepotData struct {
	AvatarSkillDepotId uint32 `csv:"ID"`
	// 元素爆发
...
func (g *GameDataConfig) loadAvatarSkillDepotData() {
	g.AvatarSkillDepotDataMap = make(map[uint32]*AvatarSkillDepotData)
...
func GetAvatarEnergySkillConfig(skillDepotId uint32) *AvatarSkillData {
	// 角色技能库配置
	avatarSkillDepotDataConfig, exist := CONF.AvatarSkillDepotDataMap[skillDepotId]
	if !exist {
		return nil
	}
	// 角色充能技配置
